  ...
  if (defined($genome)) {
	...
	foreach my $row (@{$features}) {
		...
		for (my $j=0; $j < @{$complist}; $j++) {
			my $query = "\\#.*".$complist->[$j];
			$query =~ s/\s/\\s/g;
			if (defined($row->[6]) && $row->[6] =~ m/$query/) {
...
sub get_Diagram {
	...
	foreach my $subset (@{$d->item_ids_of_type('role_and')}) {
	  ...
	  foreach my $r ($subsystem->get_subsetC_roles($subset)) {
	    ...
	    $note .= "<li>$r</li>";
	    $lookup->{$r_abbr}->{role_and}->{$subset} = 1;
	...
	foreach my $subset (@{$d->item_ids_of_type('role_or')}) {
	  ...
	  foreach my $r ($subsystem->get_subsetC_roles($subset)) {
	    ...
	    $note .= "<li>$r</li>";
	    $lookup->{$r_abbr}->{role_or}->{$subset} = 1;
	...
	foreach ( map { $subsystem->get_abbr_for_role($_) } @roles ) {
	  ...
	  if (exists($lookup->{$_})) {
	    if (defined($lookup->{$_}->{role_or})) {
	    	foreach my $subset (keys(%{$lookup->{$_}->{role_or}})) {
	    		$role_or->{$subset}->{$_} = 1;
	    	}
	    }
	    if (defined($lookup->{$_}->{role_and})) {
	    	foreach my $subset (keys(%{$lookup->{$_}->{role_and}})) {
	    		$role_and->{$subset}->{$_} = 1;
	    	}
	    }
	    next;
...
sub print_gene_popup {
    ...
    my $peg_link = "http://pubseed.theseed.org/seedviewer.cgi?page=Annotation&feature=";
    # open divs
